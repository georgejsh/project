...
\fbox{
  \parbox{\textwidth}{
  \textbf{Input:} A data Graph $D$, and Query Graph $Q$. The graphs $D$ and $Q$ are undirected with nodes and edges having label.See Figure 1.
  \\
...
\begin{tikzpicture}[node distance=2cm]
\node[circle,draw] (1)[]{B};
\node[circle,draw] (2)[below left  of=1]{D};
\node[circle,draw] (3)[below right  of=1]{E};
\path
	(1) edge node{} (2)
		 edge node{} (3);	
\end{tikzpicture}
...
\end{algorithm}
	...
	\hspace{10mm}Then CVS for each NEC is found in the data graph. Then for each combination the actual graph is tested for a match. 
